...
Messages.prototype.userAlreadyJoinedServer = function () {
  this.message.author.send(
    "Hey <@" +
      ...
      "> - you have already joined on this server! (" +
...
Messages.prototype.welcomeUser = function (channelName) {
  this.message.author.send(
    "Hey <@" +
      this.message.author.id +
      ">, welcome to ServerWordle! Send me your guesses for the 5-letter word I'm thinking of today, competing against everyone on " +
      this.message.guild.name +
      "." +
      ...
      "\nView stats by sending **!stats** in the #" +
      channelName +
      " channel of your server. Contact <@959962942718242868> with any questions." +
      "\n\nLet's play!",
  );
};
...
Messages.prototype.upToDateOnServer = function (serverName) {
  const timeUntilMidnight = moment(moment().format("YYYY-MM-DD") + " 00:00:00")
    .add(1, "days")
    .unix();
  this.user.send(
    `You are up to date on this server! (${serverName}) Try again <t:${timeUntilMidnight}:R>.`,
  );
};
...
Messages.prototype.completedAllWordles = function () {
  const timeUntilMidnight = moment(moment().format("YYYY-MM-DD") + " 00:00:00")
    ...
    .unix();
  this.message.author.send(
    `You're all caught up! Message me again <t:${timeUntilMidnight}:R> to try your next ServerWordle!`,
  );
};
...
Messages.prototype.nextWordleTime = function () {
  const timeUntilMidnight = moment(moment().format("YYYY-MM-DD") + " 00:00:00")
    ...
    .unix();
  this.message.author.send(
    `Message me again <t:${timeUntilMidnight}:R> to try your next ServerWordle!`,
  );
};

Messages.prototype.gameLogError = function () {
  this.message.author.send(
    "Error #7: game log mismatch. Please contact admin.",
  );
};

Messages.prototype.invalidFiveLetterWord = function () {
  return this.message.author.send(
    "Sorry that wasn't a valid word. Please enter a different 5-letter word guess.",
  );
};

Messages.prototype.duplicateGuess = function (word) {
  return this.message.author.send(
    `You've already guessed that word (${word}). Try another one!`,
  );
};

Messages.prototype.validGuessResponse = function (
  wordleNumber,
  serverName,
  userState,
  ...
  this.message.author.send(
    `ServerWordle #${wordleNumber} (for server ${serverName})`,
  );
  if (userState !== 1) {
    this.message.author.send({
      content: "Previous guesses:",
      components: previousGuessButtonRows,
    });
  }
  this.message.author.send({
    content: "Your guess (" + userState + "/6):",
    components: [currentGuessButtonRow],
  });
};

Messages.prototype.wonGame = function (wordleNumber, userState) {
  this.message.author.send(
    `Well done! You correctly guessed ServerWordle #${wordleNumber} in ${userState} guesses.`,
  );
};

Messages.prototype.lostGame = function (wordleNumber, wordleAnswer) {
  this.message.author.send(
    "Unlucky! You didn't get ServerWordle #" +
      wordleNumber +
...
Messages.prototype.promptCheckNewGames = function () {
  this.message.author.send(
    "Wait! ✋ You have more ServerWordles to complete! Choose the server with a reaction, or send me a message and I'll find the next game for you. 🕵️‍♂️",
  );
};
...
Messages.prototype.joinedButGameInProgress = function (serverName) {
  this.message.author.send(
    "Joined on server " +
      serverName +
      ". Don't let this distract you from your current game! Send your next guess...",
  );
};

Messages.prototype.botNotAllowed = function () {
  this.user.send(
    "I can't start a game on that server (I might have been kicked!)",
  );
};

Messages.prototype.terminatingGame = function () {
  this.user.send(
    "I can't access that server any more. Your current game will be terminated.",
  );
};
