...
WordleBot.on("guildCreate", async (guild) => {
  try {
    let channel = WordleBot.channels.cache.find(
      (e) => e.guildId === guild.id && e.name === "wordle-bot",
    );
    if (channel === undefined) {
      await guild.channels.create("wordle-bot", "text");
      channel = WordleBot.channels.cache.find(
        (e) => e.guildId === guild.id && e.name === "wordle-bot",
      );
    }
    ...
    channel
      .send(
        `Hi, I'm ServerWordle. First type **!join**, then DM me each day to play wordle against everyone on your server. View your stats with **!stats**. Admins can change my channel with **!summonserverwordle.**`,
      )
      .then((msg) => msg.pin());
    const owner = await guild.fetchOwner();
    owner.send(
      `Thanks for inviting ServerWordle! I've set up a wordle-bot text channel in your server. Head over there and type **!join** to get started! If you want to use another channel for the bot, type **!summonserverwordle** in the desired channel.`,
    );
    console.log(
    ...
    const owner = await guild.fetchOwner();
    owner.send(
      `ServerWordle needs all the requested permissions to function. Please kick and then re-invite the bot, allowing all requested permissions!`,
    );
    console.error(
      "Error: Bot added to server " +
        guild.id +
        " " +
        guild.name +
...
WordleBot.on("messageCreate", async (message) => {
  ...
  if (
    ...
    !message.author.bot &&
    message.content == "!summonserverwordle"
  ) {
    if (message.member.permissions.has(Permissions.FLAGS.MANAGE_CHANNELS)) {
      ...
      try {
        ...
        console.error(
          "Error sending welcome message and pin after !summonserverwordle for channel " +
      ...
      message.author.send(
        "You need to have the MANAGE_CHANNELS permission to use the !summonserverwordle command!",
      );
    }
  }
  ...
  if (
    ...
    const userServerGameRecords = await db.qryUserServerGameRecords(
      message.author.id,
      message.guildId,
    );
    if (userServerGameRecords.length > 0) {
      ...
      //new user for this server
      let serversWithAnswers = await db.qryServersWithAnswers();
      if (
        serversWithAnswers.filter((e) => e.server === message.guildId).length ==
        0
      ) {
        //new user and new server
        console.log(
          "Create first wordle for server id " +
            message.guildId +
            " " +
            message.guild.name,
        );
        const chosenAnswer =
          AnswerList[
            Math.floor(Math.random() * AnswerList.length)
          ].toUpperCase();
        await db.insertAnswerRow(message.guildId, chosenAnswer, 1);
        messageAdmin(
          `New server joined! ${message.guild.name} (${message.guildId}). Action performed by ${message.author.id}`,
        );
      }
      const serverLatestAnswer = await db.qryServerLatestAnswer(
        message.guildId,
      );
      const wordleNumber = serverLatestAnswer[0].wordle_number;
      const userGameLogsNotJoin = await db.qryUserGameLogsNotJoin(
        message.author.id,
      );
      if (userGameLogsNotJoin.length == 0) {
        await db.insertGameLogJoin(
          message.author.id,
          message.guild.id,
          wordleNumber,
        );
        await db.insertGameLogStart(
          message.author.id,
          message.guild.id,
          wordleNumber,
        );
        ...
        const latestGameLogNotJoin =
          userGameLogsNotJoin[userGameLogsNotJoin.length - 1];

        if (latestGameLogNotJoin.event_type == "START") {
          //user is in game
          await db.insertGameLogJoin(
            message.author.id,
            message.guild.id,
          ...
          //skip full welcome for any further servers joined
          await db.insertGameLogJoin(
            message.author.id,
            message.guild.id,
            wordleNumber,
...
WordleBot.on("messageReactionAdd", async (reaction, user) => {
  ...
  if (
    ...
    if (userGameLogs.length == 0) {
      ...
      const selectedServerIndex = serverSelectEmojis.indexOf(
        reaction.emoji.name,
      );
      ...
      if (
        ...
        selectedServerIndex >= filteredServers.length ||
        filteredServers.length === 0
      ) {
        return;
      } else {
        await tryCreateNewGameFromReaction(
          user.id,
          filteredServers[selectedServerIndex].server,
        );
      }
    }
  }
...
async function getServerNamesForUser(user) {
  ...
  let serverNames = [];
  for (const serverRow of userServersJoined) {
    try {
...
async function handleGame(message) {
  ...
  if (latestGameLogNotJoin.event_type == "START") {
    ...
    try {
      ...
      console.error(e);
      const msg = new Messages(message);
      await db.insertGameLogLose(
        message.author.id,
        latestGameLogNotJoin.server,
        latestGameLogNotJoin.wordle_number,
        "EASY",
      );
      msg.terminatingGame();
...
async function tryCreateNewGame(message, execute = true) {
  ...
  for (let serverRow of userServersJoined) {
    let newGameFound = await queryNewGameForUserServer(
      message.author.id,
      serverRow.server,
    );
    try {
...
async function queryNewGameForUserServer(userId, serverId) {
  ...
  if (
    lastWordlePlayed.length == 0 ||
    serverLatestAnswer[0].wordle_number >
      lastWordlePlayed[lastWordlePlayed.length - 1].wordle_number
  ) {
    return true;
  }
  return false;
...
async function processGameResponse(message, wordleNumber, serverId) {
  const msg = new Messages(message);
  const serverAnswer = await db.qryServerAnswerByWordleNumber(
    serverId,
    wordleNumber,
  );
  if (serverAnswer.length == 0) {
    console.log(
      "Error #7: game log mismatch for user " +
  ...
  const userSelectedGuessLog = await db.qryUserServerGuessLogSpecific(
    message.author.id,
    serverId,
    wordleNumber,
  );
  const alreadyGuessedWords = userSelectedGuessLog.map((log) =>
  ...
  switch (userState) {
    ...
    case UserStates.Guess6:
      if (!GuessList.includes(message.content.toLowerCase())) {
        ...
        msg
          .duplicateGuess(message.content.toUpperCase())
          .then((msg) => {
            setTimeout(() => msg.delete(), 5000);
          })
          .catch(console.error);
...
async function sendFrontendResponse(
  ...
  let userGuesses = [];
  let guessColours = [];
  const userGuessLog = await db.qryUserServerGuessLogSpecific(
    message.author.id,
    serverId,
    wordleNumber,
  );
  ...
  for (let i = 0; i < userGuesses.length; i++) {
    previousGuessButtonRows.push(
      generateButtons(userGuesses[i], guessColours[i], i),
    );
  }
  db.insertGuessLog(
    message.author.id,
    ...
    getColours(message.content.toUpperCase(), wordleAnswer),
  );
  userGuesses.push(message.content.toUpperCase());
  guessColours.push(getColours(message.content.toUpperCase(), wordleAnswer));
  let currentGuessButtonRow = generateButtons(
    userGuesses[userGuesses.length - 1],
    guessColours[guessColours.length - 1],
    6,
  );
  msg.validGuessResponse(
    wordleNumber,
    serverName,
    userState,
    previousGuessButtonRows,
    currentGuessButtonRow,
  );
  if (message.content.toUpperCase() == wordleAnswer) {
    msg.wonGame(wordleNumber, userState);
    const difficulty = getDifficulty(userGuesses, guessColours);
    const currentStreak =
      (await getCurrentStreak(message.author.id, serverId)) + 1;
    await db.insertGameLogWin(
      message.author.id,
      serverId,
      wordleNumber,
      userState,
    ...
    if (hasNewGame) {
      ...
      await msg.serverListWithReacts(
        await getServerNamesForUser(message.author),
      );
    } else {
      msg.nextWordleTime();
      await msg.serverListWithReacts(
    ...
    const difficulty = getDifficulty(userGuesses, guessColours);
    const lostStreak = await getCurrentStreak(message.author.id, serverId);
    await db.insertGameLogLose(
      message.author.id,
      serverId,
      wordleNumber,
...
async function getMaxStreakMedal(userId, guildId) {
  const userServerMaxWinStreak = await db.qryUserServerMaxWinStreak(
    userId,
    guildId,
  );
  let maxStreak = 0;
...
async function publishAnswer(
  ...
  let messageString =
    "ServerWordle #" + wordleNumber + " " + guessString + "/6";
  if (difficulty == "HARD") {
    messageString += "*";
  }
  ...
  try {
    ...
    WordleBot.users
      ...
      .then((user) =>
        user.send(
          `Your result has been published to the ${channel.name} channel on server ${serverName}!`,
        ),
      );
  } catch (e) {
    console.error(e);
    WordleBot.users
      ...
      .then((user) =>
        user.send(
          '"wordle-bot" channel not found on server. Could not publish result. Please contact your server admin and ask them to use the **!summonserverwordle** command in a new channel.',
        ),
      );
  }
}
...
async function refreshAnswers() {
  ...
  for (let i = 0; i < allServersLatestAnswers.length; i++) {
    ...
    if (now.diff(then.endOf("day")) > 0) {
      ...
      let pastAnswers = await db.qryServerLastNAnswers(
        allServersLatestAnswers[i].server,
        365,
      );
      let pastArr = [];
      ...
      let newWordle = AnswerList.filter((e) => !pastArr.includes(e))[
        Math.floor(Math.random() * (AnswerList.length - pastArr.length))
      ].toUpperCase();
      await db.insertAnswerRow(
        allServersLatestAnswers[i].server,
        newWordle,
        allServersLatestAnswers[i].wordle_number + 1,
      );
    }
  }
}
...
async function hasUserPlayedRecently(userId, numDays = 7) {
  ...
  const lastTime = moment(
    userGameLogsNotJoin[userGameLogsNotJoin.length - 1].date,
  );
...
async function cacheRecentUsers() {
  ...
  for (let userRow of users) {
    ...
    if (isRecentPlayer) {
      try {
        await WordleBot.users.fetch(userRow.user);
      } catch (e) {
        console.error(e);
        console.error("error caching user " + userRow.user);
      }
    }
  }
}
...
async function dailyReminder() {
  ...
  if (moment().diff(nextScheduledTime) > 0) {
    ...
    const greeting =
      reminderStrings[Math.floor(Math.random() * reminderStrings.length)];
    const users = await db.qryAllUniqueUsers();
    for (let userRow of users) {
...
function getDifficulty(guesses, colours) {
  if (guesses.length == 1) {
    return "HARD";
  } else {
    for (let i = 1; i < guesses.length; i++) {
      ...
      for (let l = 0; l < requiredPositions.length; l++) {
        if (
          letters[requiredPositions[l]] !==
          guesses[i - 1].split("")[requiredPositions[l]]
        ) {
          return "EASY";
        }
      }
...
async function getCurrentStreak(userId, serverId) {
  const userServerLastCompletedGame = await db.qryUserServerLastCompletedGame(
    userId,
    serverId,
  );
...
function getColours(guess, wordle) {
  ...
  for (let i = 0; i < answerLetters.length; i++) {
    if (
      answerLetters[i] == guessLetters[i] &&
      uncolouredLetters.includes(guessLetters[i])
    ) {
      colours[i] = "G";
  ...
  for (let i = 0; i < colours.length; i++) {
    ...
    for (let j = 0; j < 5; j++) {
      if (colours[i].charAt(j) !== "B") {
        lettersRemaining = lettersRemaining.replace(
          guesses[i].charAt(j).toLowerCase(),
          guesses[i].charAt(j),
        );
      }
    }
...
async function buildCustomChannelCache() {
  ...
  for (const customChannelTableRow of customChannels) {
    customChannelCache.set(
      customChannelTableRow.server,
      customChannelTableRow.custom_channel,
    );
  }
...
async function updateCustomChannel(serverId, newChannelId) {
  ...
  try {
    ...
    console.log(
      "Update custom channel to channel " +
        channel.name +
        " " +
        newChannelId +
        " on server " +
    ...
    console.log(
      "Error updating custom channel to channel " +
        newChannelId +
        " on server " +
        serverId +
        "\n" +
